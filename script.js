document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    fetch(action, {
      ...
      .then(function (response) {
        if (response.ok) {
          showFormMessage(form, 'success', form.dataset.successMessage || 'Your message has been sent successfully!');
          form.reset();
        } else {
      ...
      .catch(function (err) {
        showFormMessage(form, 'error', form.dataset.errorMessage || 'Something went wrong. Please try again.');
  ...
  function validateField(field) {
    ...
    if (field.hasAttribute('required') && value === '') {
      ...
      } else if (field.maxLength && field.maxLength > 0 && value.length > parseInt(field.maxLength)) {
        errorMessage = field.dataset.errorMaxlength || ('Maximum ' + field.maxLength + ' characters allowed.');
      } else if (field.pattern) {
        const regex = new RegExp('^(?:' + field.pattern + ')$');
        if (!regex.test(value)) {
          errorMessage = field.dataset.errorPattern || field.title || 'Please match the required format.';
        }
      }
    }
  ...
  function showFieldError(field, message) {
    ...
    let errorEl = document.getElementById(field.id + '-error') || field.parentElement.querySelector('.field-error');
  ...
  function clearFieldError(field) {
    ...
    const errorEl = document.getElementById(field.id + '-error') || field.parentElement.querySelector('.field-error');
    if (errorEl) {
      errorEl.textContent = '';
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "ים ויער משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: 'ce34cab5-2549-49bc-bb4a-d799784e3cb0',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || 'ce34cab5-2549-49bc-bb4a-d799784e3cb0';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'ce34cab5-2549-49bc-bb4a-d799784e3cb0';
